...
#from secrets_1 import REVIEW_API_KEY
...
#text_sample_document_df = pd.read_csv('gmap_csv_data/text_sample_document.csv')
...
if st.session_state.page == "main":
    ...
    if gmap_id1:
        #try:
            # 입력한 gmap_id1에 대한 정보 조회
            result_info = gmap_id1_info_df[gmap_id1_info_df['gmap_id1'] == gmap_id1].squeeze()

            if not result_info.empty:
                address1, gmap_id1, avg_rating1, description1, latitude1, longitude1, name1, num_of_reviews1, price1, state1, url1, main_category1, first_main_category1, region1 = (
                    result_info['address1'], result_info['gmap_id1'], result_info['avg_rating1'], result_info['description1'],
                    result_info['latitude1'], result_info['longitude1'], result_info['name1'], result_info['num_of_reviews1'], 
                    result_info['price1'], result_info['state1'], result_info['url1'], result_info['main_category1'], 
                    result_info['first_main_category1'], result_info['region1']
                )

                # 추천 결과 생성
                item_recommend_list, hybrid_recommend_list, review_recommend_list, recommendations = [], [], [], []

                # 리뷰 텍스트에서 gmap_id2 및 cosine_top4 조회
                review_info = text_sample_df[text_sample_df['gmap_id1'] == gmap_id1].sort_values(by='rank', ascending=False).head(1).squeeze()
                if not review_info.empty:
                    review_gmap2 = review_info['gmap_id2']
                    cosine_top4 = review_info['cosine_top4']

                    # gmap_id2에 대한 정보 조회 (name1과 다른 장소)
                    
                    similar_place_info = gmap_id2_info_df[(gmap_id2_info_df['gmap_id2'] == review_gmap2)].squeeze()

                    if not similar_place_info.empty:
                        address2, gmap_id2, avg_rating2, description2, latitude2, longitude2, name2, num_of_reviews2, price2, state2, url2, main_category2, first_main_category2, region2 = (
                            similar_place_info['address2'], similar_place_info['gmap_id2'], similar_place_info['avg_rating2'], 
                            similar_place_info['description2'], similar_place_info['latitude2'], similar_place_info['longitude2'], 
                            similar_place_info['name2'], similar_place_info['num_of_reviews2'], similar_place_info['price2'], 
                            similar_place_info['state2'], similar_place_info['url2'], similar_place_info['main_category2'], 
                            similar_place_info['first_main_category2'], similar_place_info['region2']
                        )

                    # 리뷰 문서 조회
                    # document_info = text_sample_document_df[(text_sample_document_df['gmap_id2'] == review_gmap2) & (text_sample_document_df['gmap_id1'] == gmap_id1)].squeeze()
                    # if not document_info.empty:
                    #     document1, document2 = document_info['document1'], document_info['document2']
            ...
            st.write('--------')
            st.write('문서의 크기와 api이용 금액으로 인한 예시 데이터 생성')            
